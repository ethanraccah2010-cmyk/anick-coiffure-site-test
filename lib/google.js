...
/* Crée l'événement de réservation et renvoie son id Google.
   opts :
     dateISO, heure, dureeMin   -> position et durée du rendez-vous
     prestation (objet config)  -> pour le libellé affiché
     cle                        -> clé stable de la prestation (config.js)
     prenom, nom, email, telephone, adresse, note -> coordonnées cliente
     source                     -> "client" ou "commercant"
   Les coordonnées sont stockées DEUX fois :
     - dans extendedProperties.private (source fiable lue par le dashboard) ;
     - en résumé lisible dans la description (si on ouvre quand même l'agenda). */
async function creerEvenement(opts) {
  const {
    dateISO, heure, dureeMin, prestation, cle,
  ...
  const description = [
    ...
    source === 'commercant' ? 'Ajouté par le salon (téléphone)' : '',
    note ? '\nNote :\n' + note : '',
  ].filter(Boolean).join('\n');

  // extendedProperties.private n'accepte que des chaînes : on normalise tout.
  const prive = {
    ...
    note: String(note || ''),
    prestation: String(cle || ''),
    source: source === 'commercant' ? 'commercant' : 'client',
  };
  ...
  const { data } = await cal.events.insert({
    ...
    requestBody: {
      ...
      extendedProperties: { private: prive },
    },
  });
  return data.id;
}

/* Liste les événements d'une plage (instants ISO) pour le dashboard commerçant.
   Renvoie les items bruts de l'API Google, déjà filtrés des annulés et des
   événements marqués « disponible » (même logique qu'evenementsDuJour).
   Le mapping en objets lisibles est fait par /api/rdv-list. */
async function listerEvenements(dateDebutISO, dateFinISO) {
  const cal = getCalendar();
  const { data } = await cal.events.list({
    calendarId: calendarId(),
    timeMin: dateDebutISO,
    timeMax: dateFinISO,
    singleEvents: true,      // déplie les événements récurrents
    orderBy: 'startTime',
    maxResults: 2500,
  ...
  return (data.items || []).filter(function (ev) {
    if (ev.status === 'cancelled') return false;        // annulé : ignoré
    if (ev.transparency === 'transparent') return false; // « disponible » : ignoré
    return true;
  });
}

/* Supprime un événement de l'agenda par son id (annulation côté commerçant). */
async function supprimerEvenement(id) {
  const cal = getCalendar();
  await cal.events.delete({ calendarId: calendarId(), eventId: id });
}
